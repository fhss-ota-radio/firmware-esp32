...
| 2026-08-12 | Qt 앱의 OTA 스캔 응답용 `EV_OTA_DISCOVER_RX` 추가 — **상태 전이 없이** `MENU_OTA`에서만 ACK(`device_id`+펌웨어 버전)를 준비하는 부수효과 이벤트로 설계 | OTA_DISCOVER(1바이트: type=6)를 받으면 기기가 자신의 고유번호로 응답해야 하는데, 이건 "다른 상태로 넘어가는 것"이 아니라 "같은 상태에서 반응만 하는 것"이라 전이표(§4)에 넣지 않고 `fsm_task()`가 `EV_ERROR`/`EV_SYNC_LOST`처럼 전이표 조회 전에 특별 처리(단, 전역이 아니라 `MENU_OTA`로 스코프 한정). ACK는 type=7 + `components/device_id`(MAC 뒤 3바이트) + `main/firmware_version.h`(major/minor/patch)를 사용하며 공통 프로토콜 version 필드는 두지 않는다. 실제 RF 송수신은 `rf_transport`가 없어 여전히 TODO, 인터페이스(`fsm_post_ota_discover_frame()`)만 먼저 정의 |
...
| `EV_OTA_DISCOVER_RX` | CC1101 수신 태스크 (`fsm_post_ota_discover_frame()`) | Qt 앱의 OTA_DISCOVER 스캔 패킷(1바이트) 수신. **상태 전이 없음** — `MENU_OTA`일 때만 `fsm_task()`가 특별 처리해 ACK(type+`device_id`+펌웨어 버전) 준비, 그 외 상태면 무시 |
...
- 코드: [`main/fsm.h`](../main/fsm.h), [`main/fsm.c`](../main/fsm.c) — 테이블 기반 상태기계, FreeRTOS 큐로 이벤트 수신. **이 파일은 애플리케이션 동작 모드만 다루며, FHSS 홉 타이밍 보정 자체는 구현하지 않는다.**
  ...
  - **OTA 스캔 ACK 구조 선반영(2026-08-12)**: Qt 앱 -> ESP `OTA_DISCOVER`(type=6, 1바이트, `components/ota_client/include/ota_discover_packet.h`)를 `fsm_post_ota_discover_frame(data, len)`로 디코드해 성공 시 `FSM_EVENT_OTA_DISCOVER_RX`를 올린다(`fsm_post_rx_audio_frame()`과 동일 패턴 — 실 호출자는 `rf_transport` 생기기 전까지 없음). `fsm_task()`가 이 이벤트를 `MENU_OTA` 상태에서만 특별 처리(`handle_ota_discover_ack()`)해 type=7 + `components/device_id`(MAC 뒤 3바이트, Little Endian) + `main/firmware_version.h`(major/minor/patch)를 담은 `OTA_DISCOVER_ACK`(7바이트)를 인코딩까지만 해둔다 — 실제 RF 송신은 TODO(팀2).
