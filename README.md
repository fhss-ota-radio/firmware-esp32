...
- [x] **`display_ui`/`ptt_button`/`rotary_encoder`/`audio_io` → FSM wiring 완료** (`main/fsm.c`)
  ...
  - `on_enter_tx_audio()`: 태스크 시작하자마자 스피커 켜고 `audio_io_play_beep()`로 삐빅음 재생 후 다시 끔(앰프 테스트용) → 이후 20ms 주기로 `audio_io_capture_encode()` 호출하는 캡처 루프(스택 8192). PTT_RELEASE로 `MENU_IDLE` 재진입 시(`on_enter_menu_idle()`) 태스크 정리. **실기기 검증 완료** — PTT 눌러서 `MENU_IDLE → TX_AUDIO` 실제 전이 확인
